...
void System::save() const 
{
    ...
    for (const auto& user : users)
    {
        ...
        if (user->getType() == UserType::Player)
        {
            ...
            for (unsigned i = 1; i <= PRODUCTS_SIZE; i++)
            {
    ...
    for (unsigned i = 1; i <= PRODUCTS_SIZE; i++)
    {
        ...
        file << market.getQuantity(type) << "\n"; 
        file << market.getPrice(type) << "\n"; 
    }
    ...
    {
        file << task.getId() << "\n";
        file << static_cast<int>(task.getProductType()) << "\n";
        file << task.getQuantity() << "\n";
        file << task.getRewardBalance() << "\n";
        file << task.getRewardScore() << "\n";
    }
...
void System::load() 
{
    ...
    for (size_t i = 0; i < userCount; ++i)
    {
        ...
        {
            users.push_back(std::make_shared<TaskManager>(username, password));
        }
        else if (type == static_cast<int>(UserType::MarketManager))
        {
            users.push_back(std::make_shared<MarketManager>(username, password));
        }
    }
    ...
    for (unsigned i = 1; i <= 8; ++i)
    {
        ...
        double price;
        file >> quantity >> price;
        market.setQuantity(static_cast<ProductType>(i), quantity);  
        market.setPrice(static_cast<ProductType>(i), price);    
    }
    ...
    for (size_t i = 0; i < taskCount; ++i)
    {
        ...
        taskBoard.addTaskFromSave(id, static_cast<ProductType>(productType),
